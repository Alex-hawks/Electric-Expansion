...
public class TileEntityTransformerT3 extends TileEntityElectricityReceiver implements IRotatable, IPacketReceiver
{
	...
	{
		ElectricityConnections.registerConnector(this, EnumSet.of(ForgeDirection.getOrientation(this.getBlockMetadata() - BlockTransformer.TIER_3_META + 2), ForgeDirection.getOrientation(this.getBlockMetadata() - BlockTransformer.TIER_3_META + 2).getOpposite()));
	...
	public void updateEntity()
	{
		...
		if (this.ticks % 20 == 0)
		{
			...
			{
				ForgeDirection inputDirection = ForgeDirection.getOrientation(this.getBlockMetadata() - BlockTransformer.TIER_3_META + 2).getOpposite();
				TileEntity inputTile = Vector3.getTileEntityFromSide(this.worldObj, new Vector3(this), inputDirection);

				// Check if requesting power on output
				ForgeDirection outputDirection = ForgeDirection.getOrientation(this.getBlockMetadata() - BlockTransformer.TIER_3_META + 2);
				...
				if (network != null && inputNetwork != null && network != inputNetwork)
				{
					...
					if (network.getRequest().getWatts() > 0)
					{
						...
						if (inputNetwork.getProduced().getWatts() > 0)
						{
							...
							double newVoltage = actualEnergy.voltage + 240;

							if (!stepUp)
								newVoltage = actualEnergy.voltage - 240;
	...
	{
		return ForgeDirection.getOrientation(this.getBlockMetadata() - BlockTransformer.TIER_3_META);
	}
