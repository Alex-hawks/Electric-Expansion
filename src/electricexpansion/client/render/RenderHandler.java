...
public class RenderHandler implements ISimpleBlockRenderingHandler
{
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer)
	{
		...
		{
			GL11.glBindTexture(3553, FMLClientHandler.instance().getClient().renderEngine.getTexture(ElectricExpansion.TEXTURE_NAME_PREFIX + "wiremill.png"));
		...
		{
			if (metadata >= 8)
				GL11.glBindTexture(3553, FMLClientHandler.instance().getClient().renderEngine.getTexture(ElectricExpansion.TEXTURE_NAME_PREFIX + "transformer3.png"));
			else if (metadata >= 4)
				GL11.glBindTexture(3553, FMLClientHandler.instance().getClient().renderEngine.getTexture(ElectricExpansion.TEXTURE_NAME_PREFIX + "transformer2.png"));
			else
				GL11.glBindTexture(3553, FMLClientHandler.instance().getClient().renderEngine.getTexture(ElectricExpansion.TEXTURE_NAME_PREFIX + "transformer1.png"));
