...
public class ContainerWireMill extends Container
{
	...
	public ContainerWireMill(InventoryPlayer par1InventoryPlayer, TileEntityWireMill tileEntity)
	{
		...
		this.addSlotToContainer(new SlotFurnace(par1InventoryPlayer.player, tileEntity, 1, 108, 25)); //Smelting result
		int var3;

		for (int i=0; i < 3; i++) //To be smelted
			this.addSlotToContainer(new Slot(tileEntity, i+2, 37 + i*18, 25));
		for (int i=0; i < 3; i++) //To be smelted
			this.addSlotToContainer(new Slot(tileEntity, i+5, 37 + i*18, 43));
